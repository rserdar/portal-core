...
| ✅ B6 | **Worker timeout koruması** — `fetchFromGas` çağrılarına `AbortController` + dinamik timeout. **Kural:** Varsayılan 20s; `uploadFile`, `generateIso`, `bulkSync` gibi ağır işlemler için 120s. GAS yanıt vermezse retry yapılmaz, `sync_log`'a CRASH yazılır | Timeout tetiklendiğinde Worker 504 yerine JSON `{ success: false, error: "GAS_TIMEOUT" }` döner |
