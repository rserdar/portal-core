...
| ✅ B6 | **Worker timeout koruması** — `fetchFromGas` çağrılarına `AbortController` + 20 saniye timeout. ⚠️ **Kural:** 20s eşiği sabit; GAS'ın yavaş çalıştığı durumda retry yapılmaz, `sync_log`'a CRASH yazılır | Timeout tetiklendiğinde Worker 504 yerine JSON `{ success: false, error: "GAS_TIMEOUT" }` döner |
